 ...
 *
 *  @param none
 *  @return console_logs (success some data, failure NULL).
 */
...
char* collect_console_logs()
{
	...
	strcat(console_logs, "\n");
	strcat(console_logs, "AssistBoardClose");
...
int clear_console_logs(int sockfd)
{
	...
	write_socket("Assist : clear_console_logs() pass. AssistBoardClose", sockfd);
...
int reboot_assist_board(int sockfd)
{
	...
	if(0 == retVal)
	{
		...
		sprintf(tmpBuf, "Assist : Board rebooting. Return value is %d. AssistBoardClose", retVal);
	...
	else
	{
		...
		sprintf(tmpBuf, "Assist : Board reboot fail. Return value is %d. AssistBoardClose", retVal);
		write_socket(tmpBuf, sockfd);
		retVal = -1;
	}
	return retVal;
...
int health_check_assist_board(int sockfd)
{
	...
	write_socket("Assist : Health check is OK. AssistBoardClose", sockfd);
	return 0;
}
...
int start_process(char* request, int sockfd)
{
	...
	if(0 == retVal)
	{
		...
		sprintf(tmpBuf, "Assist : Process %s started successfully. Return value is %d. AssistBoardClose", &request[13], retVal);
		write_socket(tmpBuf, sockfd);
		retVal = 0;
	}
	else
	{
		printf("\nAssist : Process %s started successfully\n", &request[13]);
		sprintf(tmpBuf, "Assist : Process %s failed to start. Return value is %d. AssistBoardClose", &request[13], retVal);
		write_socket(tmpBuf, sockfd);
		retVal = -1;
	}
	return retVal;	
...
int check_process_running(char* request, int sockfd)
{
	...
	if(0 == system(tmpBuf))
	{
		...
		sprintf(tmpBuf, "Assist : Process \"%s\" exist. AssistBoardClose", &request[20]);
		write_socket(tmpBuf, sockfd);
		retVal = 0;
	}
	else
	{
		...
		sprintf(tmpBuf, "Assist : Process \"%s\" do not exist. AssistBoardClose", &request[20]);
		write_socket(tmpBuf, sockfd);
		retVal = -1;
	}
	return retVal;
...
int kill_running_process(char* request, int sockfd)
{
	...
	if(0 == retVal)
	{
		...
		sprintf(tmpBuf, "Assist : Process \"%s\" killed. Return value is %d. AssistBoardClose", &request[19], retVal);
		write_socket(tmpBuf, sockfd);
		retVal = 0;
	}
	else
	{
		...
		sprintf(tmpBuf, "Assist : Process \"%s\" do not exist or not killed. Return value is %d. AssistBoardClose", &request[19], retVal);
		write_socket(tmpBuf, sockfd);
		retVal = -1;
	}
	return retVal;
...
{	
	...
	if(retVal !=0)
	{
		...
		sprintf(tmpBuf, "Assist : Execution of %s fail. Return value is %d. AssistBoardClose\n", request, retVal); 
		write_socket(tmpBuf, sockfd);
	}
	else
	{
		printf("\nAssist : Execution of %s pass. Return value is %d\n", tmpBuf, retVal);
		sprintf(tmpBuf, "Assist : Execution of %s pass. Return value is %d. AssistBoardClose\n", request, retVal);
		write_socket(tmpBuf, sockfd);
	}
	return retVal;
}